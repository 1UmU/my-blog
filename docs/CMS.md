...
- `文章图片`：集中上传、查看和删除文章使用的图片。
- `站点图片`：集中管理头像、Logo 和背景图等外观资源。
- `站点设置 -> 站点外观`：选择个人头像、导航栏 Logo、桌面背景、手机背景和默认文章封面。
- 文章编辑器中的 `封面` 字段可以直接选择或上传图片；正文工具栏中的图片按钮也使用文章图片库。

站点外观保存到 `src/config/siteAppearance.json`。保存后需要等待 GitHub Actions
完成构建，新的图片才会在线上生效。删除媒体库中的图片前，应先确认文章或站点外观不再引用该文件。
